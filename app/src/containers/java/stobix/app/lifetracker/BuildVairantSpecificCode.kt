...
class BuildVairantSpecificCode {
    companion object {
        @JvmStatic fun handleMenu(ctx:MainActivity, item:Int): Pair<Boolean, Boolean>{
            return when(item) {
                else ->
                    false to true // tell the parent method to call super
            }

        }

        @JvmStatic fun onLoad(c:MainActivity) {
